...
- `Brain.Thalamus` — relay & arbitration layer for curiosity/value/conflict/mood signals.
...
- `tokens` — the candidate tokens/MWEs with spans and `mw` flags.
- `guards` — raw guard info (e.g. `chargram_violation`, `rejected_by_boundary` indices).
- `audit` — summarized counts: `kept_tokens`, `dropped_tokens`, `boundary_drops`, `chargram_violation`, `weak_decisions`, etc.
- `choices` / `finalists` — winners and rankings per `token_index`.
- `intent` / `confidence` — intent bias used during scoring.
- `opts` — resolved runtime config (weights, scores mode, `margin_threshold`, `mwe_fallback`, ACC tau, pMTG mode).
...
This loop encodes “tiny exploratory acts” that may or may not earn a slot in WM:
...
2. `Brain.Thalamus` listens, blends in **OFC value**, **ACC conflict**, and **Mood**, and emits a decision:  
   - Telemetry: `[:brain, :thalamus, :curiosity, :decision]` with `decision ∈ :allow | :boost | :block` and a blended score in `[0,1]`.
