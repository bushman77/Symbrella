defmodule Brain.LIFG.BoundaryGuard do
  ...
  def sanitize(tokens, sentence) when is_list(tokens) do
    tokens
    |> Enum.map(&mapify/1)
    # coerce spans to {start, end}
    |> Enum.map(&normalize_span_shape/1)
    # explicit chargram markers
    |> Enum.reject(&chargram?/1)
    # NEW: any space w/o mw: true is dropped
    |> Enum.reject(&whitespace_non_mwe?/1)
    |> Enum.filter(fn t ->
  ...
  defp valid_span?(%{span: {s, e}}) when is_integer(s) and is_integer(e) and s >= 0 and e > s,
    do: true
