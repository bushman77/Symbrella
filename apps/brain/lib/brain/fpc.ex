defmodule Brain.FPC do
  ...
  @type ctx :: %{
          optional(:acc_conflict) => number(),   # 0.0..1.0 (higher = more conflict)
          optional(:novelty) => number(),        # 0.0..1.0 (Hippocampus novelty/PE)
          optional(:ofc_value) => number(),      # 0.0..1.0 (immediate EV of current parse; lower → explore)
          optional(:max_budget) => non_neg_integer(), # cap for branch_budget (default 2)
          optional(:min_switch_ms) => non_neg_integer(), # earliest switch time (default 120)
          optional(:max_switch_ms) => non_neg_integer()  # latest  switch time (default 500)
        }

  @spec plan_branches(ctx) :: %{branch_budget: non_neg_integer(), switch_after_ms: non_neg_integer()}
