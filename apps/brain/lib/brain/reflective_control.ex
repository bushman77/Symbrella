defmodule Brain.ReflectiveControl do
  @moduledoc """
  Bounded post-draft response review.

  This module treats reflection as an inspectable control pass, not hidden
  chain-of-thought. It reviews an LLM draft against the symbolic response context and
  ...
  def review(user_text, draft_text, context) when is_binary(draft_text) and is_map(context) do
    ...
    base_issues = collect_issues(context, user_text, draft_text)
    base_status = classify(base_issues)

    critic = maybe_run_critic(user_text, draft_text, context, base_issues)
    issues = merge_issues(base_issues, map_get(critic, :issues) || [])
    status = strongest_status(base_status, map_get(critic, :status), issues)
  ...
  defp apply_decision(:repair, _user_text, draft_text, issues, _context) do
    final_text =
      draft_text
      |> repair_overclaims()
    ...
    final_text =
      if final_text == "" or
           (final_text == draft_text and needs_generic_repair_text?(issues)) do
  ...
  defp hidden_context_leak?(draft_text) when is_binary(draft_text) do
    ...
    String.contains?(t, "system prompt") or
      String.contains?(t, "hidden instruction") or
      String.contains?(t, "chain of thought")
  end
  ...
  defp needs_generic_repair_text?(issues) do
    Enum.any?(
      issues,
      &(&1 in [:self_state_repair, :ignored_symbolic_state, :unsupported_fact, :wrong_tone])
    )
  end

  defp maybe_run_critic(_user_text, _draft_text, _context, []), do: critic_skipped(:not_needed)

  defp maybe_run_critic(user_text, draft_text, context, base_issues) do
    case critic_client() do
      nil ->
        critic_skipped(:no_client)

      client ->
        do_run_critic(client, user_text, draft_text, context, base_issues)
    end
  end

  defp do_run_critic(client, user_text, draft_text, context, base_issues) do
    messages = critic_messages(user_text, draft_text, context, base_issues)

    case client.chat(messages, timeout: @critic_timeout_ms) do
      {:ok, %{content: content}} when is_binary(content) ->
        parse_critic(content)
      ...
      other ->
        critic_error({:unexpected_response, other})
    end
  rescue
    error -> critic_error(Exception.message(error))
  catch
    :exit, reason -> critic_error({:exit, reason})
  end

  defp critic_messages(user_text, draft_text, context, base_issues) do
    [
      %{
        "role" => "system",
        "content" =>
          "Review this Symbrella draft. Return JSON only with keys status, issues, confidence, critique, repair_instruction. Valid statuses: accept, repair, clarify, reject. Do not include hidden reasoning."
      },
      %{
        "role" => "user",
        "content" =>
          Jason.encode!(%{
            user_text: clamp_text(user_text),
            draft_text: clamp_text(draft_text),
            deterministic_issues: base_issues,
            intent: get_in_map(context, [:features, :intent]),
            confidence:
              get_in_map(context, [:features, :conf]) ||
                get_in_map(context, [:features, :confidence]),
            confidence_bucket: get_in_map(context, [:features, :confidence_bucket]),
            self_state: get_in_map(context, [:features, :self_state]),
  ...
  defp parse_critic(content) do
    with {:ok, %{} = decoded} <- Jason.decode(content) do
      %{
        used?: true,
        status: parse_status(map_get(decoded, :status)),
        issues: parse_issues(map_get(decoded, :issues)),
        confidence: clamp01(number(map_get(decoded, :confidence))),
        critique: clamp_text(to_string(map_get(decoded, :critique) || "")),
        repair_instruction: string_or_nil(map_get(decoded, :repair_instruction))
      }
    else
      _ -> critic_error(:invalid_json)
    end
  end

  defp critic_skipped(reason), do: %{used?: false, skipped?: true, reason: reason, issues: []}

  defp critic_error(reason),
    do: %{used?: false, error?: true, reason: inspect(reason), issues: [:critic_unavailable]}

  defp critic_client do
    client =
      Application.get_env(
        :brain,
        :reflection_critic_client,
        Application.get_env(:core, :reflection_critic_client)
      )

    cond do
      is_nil(client) -> nil
      Code.ensure_loaded?(client) and function_exported?(client, :chat, 2) -> client
      true -> nil
    end
  end

  defp parse_status(status) when status in [:accept, :repair, :clarify, :reject], do: status
  ...
  defp parse_issue(issue)
       when issue in [
              :missed_user_intent,
              :overclaimed_self_awareness,
              :unsupported_fact,
              :too_generic,
              :leaked_hidden_context,
              :ignored_symbolic_state,
              :wrong_tone,
              :unsafe,
              :low_confidence,
              :unknown_intent,
  ...
  defp parse_issue(issue) when is_binary(issue) do
    ...
    |> String.trim("_")
    |> case do
      "missed_user_intent" -> :missed_user_intent
      "overclaimed_self_awareness" -> :overclaimed_self_awareness
      "unsupported_fact" -> :unsupported_fact
      "too_generic" -> :too_generic
      "leaked_hidden_context" -> :leaked_hidden_context
      "ignored_symbolic_state" -> :ignored_symbolic_state
      "wrong_tone" -> :wrong_tone
      "unsafe" -> :unsafe
      "low_confidence" -> :low_confidence
      "unknown_intent" -> :unknown_intent
      "high_uncertainty" -> :high_uncertainty
      "self_state_repair" -> :self_state_repair
      "self_state_claim_risk" -> :self_state_claim_risk
      _ -> nil
    end
  end

  defp parse_issue(_), do: nil

  defp merge_issues(left, right) do
    (List.wrap(left) ++ List.wrap(right))
    |> Enum.uniq()
  end

  defp strongest_status(base_status, critic_status, issues) do
    [base_status, critic_status, classify(issues)]
    |> Enum.reject(&is_nil/1)
    |> Enum.max_by(&status_rank/1)
  end

  defp status_rank(:accept), do: 0
  defp status_rank(:clarify), do: 1
  defp status_rank(:repair), do: 2
  defp status_rank(:reject), do: 3
  ...
  defp emit(reflection, context) do
    emit_telemetry(
      ...
      %{
        ...
        critic_used?: get_in_map(reflection, [:critic, :used?]) == true,
  ...
  defp no_op(draft_text) do
    ...
    %{
      ...
      critic: critic_skipped(:invalid_context),
      at_ms: System.system_time(:millisecond)
    }
  end
