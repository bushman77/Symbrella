defmodule Brain.LIFG do
      ...
      case run_stage1(si2, cfg, eff_opts) do
        {:ok, si3a0, raw_choices, audit} ->
          ...
          choices0 =
            ...
            |> normalize_choices_for_downstream(si3a, eff_opts)
          ...
          # Normalize again after rerun merges (so downstream always has id/chosen_id/source/alt_ids)
          choices = normalize_choices_for_downstream(choices, si3b, eff_opts)
  ...
  defp run_stage1(si_for_stage, cfg, eff_opts)
    ...
    case Stage1Bridge.normalize_result(res) do
      {:ok, si_after, raw_choices, audit} -> {:ok, si_after, raw_choices, audit}
  ...
  # Preserves feature fields even when callers omit :features (prevents “empty features” scoring).
  ...
  defp choice_winner_id(ch) when is_map(ch) do
    ...
    case raw do
      %{} = m -> m[:id] || m["id"] || m[:chosen_id] || m["chosen_id"] || m[:lemma] || m["lemma"]
      other -> other
    end
  end
  ...
  # ── NEW: make choices “downstream-safe” (WM + Stage2 contract) ─────────────

  defp normalize_choices_for_downstream(choices, si_after, eff_opts)
       when is_list(choices) and is_map(si_after) and is_list(eff_opts) do
    slate = slate_from_si(si_after)
    ...
    Enum.map(choices, fn
      ch when is_map(ch) ->
        chp = Safe.to_plain(ch)
        idx = normalize_idx(choice_token_index(chp))
        wid = normalize_signal_id(choice_winner_id(chp))

        # Prefer explicit score; otherwise use p(top1) we already computed.
        score0 = Map.get(chp, :score) || Map.get(chp, "score")

        score =
          cond do
            is_number(score0) -> score0 * 1.0
            true -> top1_prob(chp)
          end
        ...
        chp
        |> Map.put(:token_index, idx)
        |> maybe_put_if_binary(:id, wid)
        |> maybe_put_if_binary(:chosen_id, wid)
  ...
  defp normalize_choices_for_downstream(other, _si_after, _eff_opts), do: other

  defp maybe_put_if_binary(m, _k, _v) when not is_map(m), do: m
  defp maybe_put_if_binary(m, _k, v) when is_nil(v), do: m

  defp maybe_put_if_binary(m, k, v) when is_binary(v) do
    Map.put(m, k, v)
  end
  ...
  defp control_signals(choices, si_after, scores_mode, margin_thr, eff_opts) do
    ...
    # FINAL GUARANTEE: union(winner_ids, inhibitions) == all slate ids per token (minus winner)
    {boosts2, inhibs2} = ensure_full_slate_coverage(choices, boosts, inhibs, si_after, eff_opts)

    {boosts2, inhibs2}
  end

  defp ensure_full_slate_coverage(choices, boosts, inhibitions, si_after, eff_opts)
       when is_list(choices) and is_list(boosts) and is_list(inhibitions) and is_map(si_after) do
    slate = slate_from_si(si_after)
    boost_amt = Keyword.get(eff_opts, :boost_amount, 0.05) * 1.0
    inhib_amt = Keyword.get(eff_opts, :inhib_amount, 0.02) * 1.0

    winners_by_idx =
      Enum.reduce(choices, %{}, fn ch0, acc ->
        ch = Safe.to_plain(ch0)
    ...
    have_inhib = MapSet.new(Enum.map(inhibitions, fn i -> {i[:token_index], i[:id]} end))

    add_boosts =
      winners_by_idx
      |> Enum.flat_map(fn {idx, wid} ->
        if MapSet.member?(have_boost, {idx, wid}) do
          []
        else
    ...
    add_inhibs =
      Enum.flat_map(slate, fn {raw_idx, senses} ->
        idx = normalize_idx(raw_idx)
        wid = Map.get(winners_by_idx, idx)
        ...
        |> Enum.reject(fn id -> is_binary(wid) and id == wid end)
        |> Enum.flat_map(fn id ->
          if MapSet.member?(have_inhib, {idx, id}) do
            []
          else
            [%{token_index: idx, id: id, amount: inhib_amt}]
          end
        end)
      end)
    ...
    inhibs2 =
      (inhibitions ++ add_inhibs)
      |> Enum.uniq_by(fn i -> {i[:token_index], i[:id]} end)

    {boosts2, inhibs2}
  end

  defp ensure_full_slate_coverage(_choices, boosts, inhibitions, _si_after, _eff_opts),
  ...
  defp normalize_signal_id(%{}), do: nil
  ...
  # Under scores=:none, always inhibit non-winners from the slate bucket.
  defp fallback_feedback_from_slate(si_after, choices, margin_thr, eff_opts)
    ...
    Enum.reduce(slate, %{boosts: [], inhibitions: []}, fn {raw_idx, senses}, acc ->
      ...
      if is_nil(ch) do
        ...
        margin = choice_margin(Safe.to_plain(ch))
        chosen_id = choice_winner_id(Safe.to_plain(ch))
