...
defmacro __using__(opts) do
  ...
  def configure_wm(opts) when is_list(opts), do: GenServer.call(@name, {:configure_wm, Map.new(opts)})

  @doc "Replace attention context (goals/weights)."
  @spec set_attention(map()) :: :ok
  def set_attention(ctx) when is_map(ctx), do: GenServer.cast(@name, {:set_attention, ctx})
  ...
  def focus(cands_or_si, opts \\ []), do: GenServer.call(@name, {:focus, cands_or_si, Map.new(opts)})

  @doc "Remove one (by id) or many (predicate fun) from working memory."
  @spec defocus(id() | (wm_item() -> boolean())) :: :ok
  def defocus(id_or_fun), do: GenServer.call(@name, {:defocus, id_or_fun})

  @doc "Read-only view of WM."
  @spec snapshot_wm() :: %{wm: [wm_item()], cfg: wm_cfg(), attention: map()}
  def snapshot_wm, do: GenServer.call(@name, :snapshot_wm)
...
def init(:ok) do
  ...
  def handle_call({:focus, cands_or_si, opts}, _from, state) do
    ...
    # decay current WM before adding new stuff
    ...
    {wm_next, added, removed} =
      Enum.reduce(cands0, {wm_d, 0, 0}, fn cand0, {wm_acc, a_cnt, r_cnt} ->
        # optional salience from attention
    ...
    :telemetry.execute(
      [:brain, :wm, :update],
      %{size: length(wm_next), added: added, removed: removed, capacity: cfg.capacity},
      %{}
    )
  ...
  defp decide_gate(wm, cand, attn, cfg, opts, salience) do
    # If a BasalGanglia module exists later, we’ll use it.
    if Code.ensure_loaded?(Brain.BasalGanglia) and
      ...
      # Fallback policy: allow strong items; boost if clearly goal-relevant.
      base  = (cand[:score] || cand[:activation_snapshot] || 0.0) * 1.0
      bias  = min(max(salience || 0.0, 0.0), 1.0)
      final = min(base + 0.5 * bias, 1.0)

      source = cand[:source]
      prefer_source? = source in [:runtime, :recency, :lifg]

      cond do
        prefer_source? and final >= 0.2     -> {:boost, final}
  ...
  defp normalize_candidates(%{} = si) do
    # Accept a few shapes; fall back to empty list
    cond do
  ...
  # … (keep all your existing helper functions exactly as you had them: ATL, PMTG, tokens,
  #    spans, lifg helpers, episodes, control signals, etc.)
  # (I omitted them here for brevity in the message — keep your original implementations)
  # ─────────────────────────────────────────────────────────────────────────────────────

  # Small normalizers for WM config
  defp norm_pos_int(n, d) when is_integer(n) and n > 0, do: n
  defp norm_pos_int(_, d), do: d
  defp norm_float_01(x, d) when is_number(x) and x >= 0 and x <= 1, do: x * 1.0
  defp norm_float_01(_, d), do: d
# ───────────────────────── Activation / cell helpers ────────────────────────

# Accept a variety of shapes and always return a list to iterate over.
...
# Accept a few shapes of span metadata on tokens
    ...
    def upsert(wm, item, %{merge_duplicates?: true}) do
      case Enum.split_with(wm, &same_identity?(&1, item)) do
        ...
        {[existing | tail_dups], rest} ->
          # merge with the first duplicate; drop any further dups
          merged = %{
            existing
