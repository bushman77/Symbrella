defmodule Brain.LIFG.PrimingIntegrationTest do
  ...
  test "when priming is disabled, ties resolve deterministically by candidate order" do
    si = %{
      ...
      sense_candidates: %{
        0 => [
          %{
            id: "bank|financial",
            features: %{lex_fit: 0.5, rel_prior: 0.5, activation: 0.0, intent_bias: 0.0}
          },
          %{
            id: "bank|river",
            features: %{lex_fit: 0.5, rel_prior: 0.5, activation: 0.0, intent_bias: 0.0}
    ...
    si2 =
      Brain.LIFG.disambiguate_stage1(si,
        weights: %{lex: 0.25, sim: 0.0, rel: 0.15, prag: 0.10, act: 0.10},
        scores: :top2
      )
