defmodule Brain.SelfPortraitTest do
  ...
  test "emits salience from normal Blackboard payload handling" do
    if is_nil(Process.whereis(Brain.SelfPortrait)) do
      start_supervised!({Brain.SelfPortrait, []})
    else
      :ok = Brain.SelfPortrait.reset()
    ...
    handler_id = "self-portrait-blackboard-salience-test-#{System.unique_integer([:positive])}"
    ...
    assert :ok =
             :telemetry.attach(
               ...
               fn _event, measurements, metadata, pid ->
                 send(pid, {:self_salience, measurements, metadata})
               end,
               self()
             )
    ...
    send(Process.whereis(Brain.SelfPortrait), {
      :blackboard,
      %{
        kind: :telemetry,
        event: [:brain, :pipeline, :lifg_stage1, :stop],
    ...
    assert_receive {:self_salience, %{lifg_anomalies: 1}, %{}}, 200
  end
end
