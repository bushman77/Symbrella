    ...
    <div class="mt-4 rounded-2xl border p-4">
      ...
      <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
        <div class="rounded-lg border p-3">
          ...
          <div>
            last write: {case @hippo.last do
              {at, _} when is_integer(at) ->
                "#{max(System.monotonic_time(:millisecond) - at, 0)} ms ago"
          ...
          <form phx-change="hippo_source" class="flex items-center gap-2">
            <select name="v" class="rounded-md border px-2 py-1 text-sm">
              <%= for opt <- ~w(memory db hybrid)a do %>
                <option value={opt} selected={get_in(@hippo, [:opts, :recall_source]) == opt}>
                  {opt}
                </option>
              <% end %>
            </select>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1">
            ...
            <%= if Map.has_key?(@hippo_metrics, :top_score) do %>
              <div>
                top_score:
                <b>
                  {:erlang.float_to_binary(@hippo_metrics[:top_score] * 1.0, [:compact, decimals: 3])}
                </b>
              </div>
            <% end %>
            ...
            <div class="col-span-2 md:col-span-4 text-xs opacity-60">
              {ts = @hippo_metrics[:at_ms] || 0} updated: {if ts > 0,
                do: "#{System.system_time(:millisecond) - ts} ms ago",
                else: "—"}
            </div>
          </div>
        <% else %>
