defmodule Db.Episode do
  ...
  defp normalize_si(changeset) do
    case get_change(changeset, :si) do
      ...
      %{} = si ->
        ...
        Logger.debug("Si normalized: #{inspect(Map.keys(normalized), limit: 10)}")  # Debug: Check top-level keys
  ...
  defp to_plain_map(%{__struct__: _} = struct) do
    ...
    struct
    |> Map.from_struct()
    |> Map.delete(:__meta__)  # Drop Ecto metadata
    |> Map.new(fn {k, v} ->
      {k, to_plain_map(v)}
    end)
  ...
  defp convert_non_json_types(map) when is_map(map) do
    ...
    |> Map.update(:activation_summary, %{}, fn summary ->
      Map.update(summary, :db_hits, [], fn hits ->
        to_plain_map(hits)  # Ensures MapSet → list
      end)
    end)
    |> Map.update(:trace, [], fn trace ->
      Enum.map(trace, fn item ->
        to_plain_map(item)  # Handles tuples → lists or maps
      end)
    end)
    |> Map.update(:active_cells, [], fn cells ->
      Enum.map(cells, &to_plain_map/1)  # Ensures BrainCell structs → plain maps
    end)
  end
